...
export const useProveedor = () => {
    ...
    const detalle = async (id: number) => {
        try {
            proveedor.value = await ProveedorApi.detalleProveedor(id);
    ...
    const eliminar = async (id_proveedor: number) => {
        try {
            await ProveedorApi.eliminar(id_proveedor);
            if (!route.params.id) {
                window.location.reload();
            }
            router.push('/proveedores');
        } catch (error) {
            console.log('Fallo al llamar a la API', error);
        }
    };

    const modificar = async (nuevo: Proveedor) => {
        try {
            await ProveedorApi.update(nuevo);
            router.push('/proveedores');
        } catch (error) {
    ...
    const crear = async (nuevo: Proveedor) => {
        try {
            await ProveedorApi.crear(nuevo);
            router.push('/proveedores');
        } catch (error) {
            console.log(error);
        }
